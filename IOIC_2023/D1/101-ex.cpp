...
using PTF = complex<llf>;//PTF=座標變數
...
struct L {//一條線
  PTF st, ed, dir;//a點,b點,兩線距
  L(PTF s, PTF e) : st(s), ed(e), dir(e - s) {}
};

vector<PTF> interCircle(const L &l, const PTF &c, llf r) {//求焦點 interCircle(線,圓心,半徑)
...
int main() {
  ...
  PTF c(x0, y0);
  auto p = interCircle(l, c, r);//線,圓心,半徑
  if (p.empty()) {
    cout << "none\n";
  } else {
    for (auto pi : p)
